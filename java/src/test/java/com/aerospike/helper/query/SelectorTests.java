...
//	}
	...
	public void selectWithGeoWithin() throws IOException{
		...
		String rgnstr = String.format("{ \"type\": \"AeroCircle\", "
							  + "\"coordinates\": [[%.8f, %.8f], %f] }",
							  lon, lat, radius);
		System.out.println("GEO_Query: " + rgnstr);
		Statement stmt = new Statement();
		stmt.setNamespace("test");
		stmt.setSetName(geoSet);
		PredExp[] predexps = new PredExp[3];
		try {
		predexps[0]= PredExp.geoJSONBin(geoBinName);
		predexps[1] = PredExp.geoJSONValue(rgnstr);
		predexps[2] = PredExp.geoJSONWithin();
		stmt.setPredExp(predexps);
//		RecordSet rs = client.query(null, stmt);
//			stmt.setPredExp(qual1.toPredExp().toArray(new PredExp[0]));
//		queryEngine.refreshCluster();
//		KeyRecordIterator it = new KeyRecordIterator(stmt.getNamespace(), rs);
//		System.out.println(Value.getAsGeoJSON(rgnstr).toString());
